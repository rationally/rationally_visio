    ...
    internal sealed class ForceContainer : HeaderlessContainer
    {
            ...
            if (Children.Count == 0)
            {
                foreach (Shape shape in shapes)
                {
                    ...
                    else if (ForceValueComponent.IsForceValue(shape.Name))
                    {
                        ...
                        Children.Add(comp);
                        forceValuesDictionary.Add(comp.ForceAlternativeUniqueIdentifier, comp.Text);
                    }
                }
        ...
        public override void Repaint()
        {
            ...
            foreach (Alternative alt in alternatives)
            {
                ...
                ForceValueComponent altValue = (ForceValueComponent)Children.FirstOrDefault(c => c is ForceValueComponent && !c.Deleted && (((ForceValueComponent)c).ForceAlternativeUniqueIdentifier == alt.UniqueIdentifier));
            ...
            List<ForceValueComponent> toRemove = alreadyThere.Where(f => !f.Deleted && !alternatives.ToList().Any(alt => alt.UniqueIdentifier == f.ForceAlternativeUniqueIdentifier)).ToList();
            List<ForceValueComponent> toRemoveFromTree = alreadyThere.Where(f => f.Deleted || !alternatives.ToList().Any(alt => alt.UniqueIdentifier == f.ForceAlternativeUniqueIdentifier)).ToList();
